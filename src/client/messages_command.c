...
static int messages_command_print_lines(client_t *client)
{
    line_t *line = NULL;
    int ret = 0;
...
static int messages_command_end(client_t *client, line_t *answer)
{
    write(1, answer->buf, answer->len);
    if (answer->len > 0 && answer->buf[0] == '1') {
        messages_command_print_lines(client);
    }
    free(answer->buf);
    free(answer);
    return 0;
...
int messages_command(client_t *client, client_info_t *info, char **args)
{
    ...
    if (ret != 0)
        return ret;
    return messages_command_end(client, line);
}
