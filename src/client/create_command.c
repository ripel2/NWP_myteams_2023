...
static int create_command_cleanup(client_t *client)
{
    line_t *line = NULL;
...
static int create_command_end(client_t *client, line_t *line, char *msg)
{
    int ret = 0;
    char *msgline = NULL;
    ...
    return create_command_cleanup(client);
}
...
int create_command(client_t *client, client_info_t *info, char **args)
{
    ...
    if (ret != 0)
        return ret;
    return create_command_end(client, line, args[2]);
}
