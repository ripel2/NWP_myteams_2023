...
static int subscribe_command_end(line_t *answer, char *user, char *team)
{
    ...
    if (strncmp(answer->buf, "530", 3) == 0) {
        ...
        dll.functions[CLIENT_PRINT_SUBSCRIBED](user, team);
    }
...
int subscribe_command(client_t *client, client_info_t *info, char **args)
{
    ...
    if (ret != 0)
        return ret;
    return subscribe_command_end(line, info->username, args[1]);
}
